# FISCO-BCOS 文档导航

本页整理了 FISCO-BCOS **2.0 / 3.0** 的核心文档入口、搭建指南、WeBASE 部署文档以及通用兼容性说明，方便快速查阅。

---

## 2.0 文档

### 📘 基础文档  
- FISCO-BCOS 2.0 文档（旧版入口）  
  [https://fisco-bcos-doc-chinese.readthedocs.io/zh-cn/release-2.0.0/docs/manual/index.html](https://fisco-bcos-doc-chinese.readthedocs.io/zh-cn/release-2.0.0/docs/manual/index.html)

### 🏗️ 搭建第一个区块链网络  
- 官方安装指南  
  [https://fisco-bcos-documentation.readthedocs.io/zh-cn/latest/docs/installation.html](https://fisco-bcos-documentation.readthedocs.io/zh-cn/latest/docs/installation.html)

### 🧰 WeBASE（2.0）  
- WeBASE 一键部署平台  
  [https://webasedoc.readthedocs.io/zh-cn/lab/docs/WeBASE-Install/developer.html](https://webasedoc.readthedocs.io/zh-cn/lab/docs/WeBASE-Install/developer.html)

### 🌐 区块链浏览器部署（2.0）  
- WeBASE-Browser 部署  
  [https://fisco-bcos-doc-chinese.readthedocs.io/zh-cn/release-2.0.0/docs/browser/deploy.html](https://fisco-bcos-doc-chinese.readthedocs.io/zh-cn/release-2.0.0/docs/browser/deploy.html)

---

## 3.0 文档

### 📘 基础文档  
- 快速开始（AIR 安装）  
  [https://fisco-bcos-doc.readthedocs.io/zh-cn/latest/docs/quick_start/air_installation.html](https://fisco-bcos-doc.readthedocs.io/zh-cn/latest/docs/quick_start/air_installation.html)  
- 教程入口  
  `https://fisco-bcos-doc.readthedocs.io/zh-cn/latest/docs/tutorial/air/index.html` [(fisco-bcos-doc.readthedocs.io in Bing)](https://www.bing.com/search?q="https%3A%2F%2Ffisco-bcos-doc.readthedocs.io%2Fzh-cn%2Flatest%2Fdocs%2Ftutorial%2Fair%2Findex.html")

### 🏗️ 搭建第一个区块链网络  
- build_chain 工具搭建  
  `https://fisco-bcos-doc.readthedocs.io/zh-cn/latest/docs/operation_and_maintenance/build_chain.html` [(fisco-bcos-doc.readthedocs.io in Bing)](https://www.bing.com/search?q="https%3A%2F%2Ffisco-bcos-doc.readthedocs.io%2Fzh-cn%2Flatest%2Fdocs%2Foperation_and_maintenance%2Fbuild_chain.html")

### 🧰 WeBASE（3.0）  
- WeBASE 一键部署区块链  
  [https://webasedoc.readthedocs.io/zh-cn/lab/docs/WeBASE/install.html](https://webasedoc.readthedocs.io/zh-cn/lab/docs/WeBASE/install.html)

---

## 通用文档

### 🔧 版本兼容性说明  
- FISCO-BCOS 兼容性文档  
  [https://fisco-bcos-documentation.readthedocs.io/zh-cn/latest/docs/compatibility.html](https://fisco-bcos-documentation.readthedocs.io/zh-cn/latest/docs/compatibility.html)

---
