...
interface MedicalRecord {
    ...
    createTime: string;
    isFilled: boolean;
    registrationInfo: string; // 添加registrationInfo字段
}
...
const patient = localStorage.getItem('accountAddress') || '{}';
...
const formatDateTime = (timestamp: string) => {
    ...
    return date.toLocaleString() != "Invalid Date" ? date.toLocaleString() : null;
};

const fetchMedicalRecords = async () => {
    try {
        const response = await axios.get('http://localhost:8085/getMedicalRecordList', { 
            params: { 
                address: patient,
        ...
        medicalRecords.value = response.data.map((record: MedicalRecord) => ({
            ...record,
...
const viewRecord = (row: MedicalRecord) => {
    Object.assign(selectedRecord, row);// Copy the record to the temporary variable
    // selectedRecord = record; // Keep the original reference if needed
    viewOnly.value = row.isFilled; // Set viewOnly based on the record's status
