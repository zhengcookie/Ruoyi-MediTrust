...
onMounted(() => {
    doctorAddress.value = localStorage.getItem('accountAddress') || '{}';
    console.log(doctorAddress.value);
    axios.get('http://localhost:8085/getInfo', { params: { address: doctorAddress.value } }).then((res) => {
        // PersonalInformation.id = res.data.msg.id;
        PersonalInformation.username = res.data.name;
        PersonalInformation.gender = res.data.gender;
        PersonalInformation.age = res.data.age;
        PersonalInformation.accountAddress = res.data.accountAddress;
